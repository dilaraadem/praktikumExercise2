...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	private void initializeGroupsOfInitialPedestrians() {
		...
		if (c.getElements().size() > 0) {
			...
			SIRGroup g1 = getNewGroup(SIRType.ID_SUSCEPTIBLE.ordinal(), Integer.MAX_VALUE/2);
			SIRGroup g2 = getNewGroup(SIRType.ID_INFECTED.ordinal(), Integer.MAX_VALUE/2);
			SIRGroup g3 = getNewGroup(SIRType.ID_REMOVED.ordinal(), Integer.MAX_VALUE/2);

			groupsById.put(SIRType.ID_SUSCEPTIBLE.ordinal(), g1);
			groupsById.put(SIRType.ID_INFECTED.ordinal(), g2);
			groupsById.put(SIRType.ID_REMOVED.ordinal(), g3);
	...
	public void update(final double simTimeInSec) {
		...
		if(simTimeInSec - last_update >= 1){
			...
			/*for(Pedestrian p : c.getElements()) {
				...
				LinkedCellsGrid grids = c.getCellsElements();
				for(Pedestrian p : c.getElements()){
					...
					if(getGroup(p).getID() == SIRType.ID_INFECTED.ordinal()){
						List<Pedestrian> infectlist = grids.getObjects(p.getPosition(), attributesSIRG.getInfectionMaxDistance());
						for(Pedestrian p_neighbor: infectlist){
							if(this.random.nextDouble() < attributesSIRG.getInfectionRate()){
								SIRGroup g = getGroup(p_neighbor);
								if(g.getID() == SIRType.ID_SUSCEPTIBLE.ordinal()){
									elementRemoved(p_neighbor);
									assignToGroup(p_neighbor, SIRType.ID_INFECTED.ordinal());
								}
							}
						}
					}
